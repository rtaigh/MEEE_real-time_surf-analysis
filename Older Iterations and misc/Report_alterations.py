...
cap = cv2.VideoCapture('masters_Video_cap//Pipeline.mp4')#'masters_Video_cap//19_1_2019_1.mp4')#31_12_2018_3.mp4')#6_1_2018_4.mp4')#6_1_2018_3.mp4')#28_12_2018_8.mp4')#29_12_2018_0.mp4')#'6_1_2018_4.mp4')#19_1_2019_1.mp4')#'My Video1.mp4')#     ITERATION THEN SHIFT AMOUNT
...
#function for convex hull and centre of object
def convex_hull (frame,yt,ht,colour,centroid):#, #y_val):|
    ...
    area=0    
    for c in contours:
        #find largest area in the frame
    ...
    for c in contours:
        ...
        if cv2.contourArea(c)==area:
            ...
            #input (Don't want centroid on swell coloring)
            if centroid==True:
   ...
   # tracker(base_cut+10,BB_size,threshold_y,roi_base)
    ...
    for contour in contours:
          ...
          if wcon>40 and wcon*hcon>100 and wcon>1280/10:#width/10 :#and hcon<30:
              ...
              if max_line>=min_line and max_line+max_base<=min_line+minbase :
                  max_line=min_line
                  max_base=minbase
                  
              if min_line >= max_line and min_line+minbase<=max_line+max_base :
                  min_line=max_line
                  minbase=max_base
...
while True:
    ...
    try:
       ...
       # frame=frame[base_cut:roi_base,0:1280]
        ...
        #HowToGetSeaSick
        cv2.imshow('frame Cropped',frame)
    except:
    ## Generic RoI
        frame=frame[0:roi_base,0:1280]
  

    #Further filtering         #grey scale
 ...
 #   Occlusion_counter()
    ...
    cv2.imshow('Gray_scale frame',frame)
