    ...
    for c in contours:
        if cv2.contourArea(c)==area:
            ...
            if centroid==True:
                ...
                cv2.putText(r_frame, "Moment", (cX - 25, base_cut+cY - 25),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def FiFo_brackets(remov,blw_avg,occlusion_counter,abv_avg, max_thres,min_thres):
  ...
  #  cv2.line(r_frame,(0,base_cut+max_line+max_base),(width,base_cut+max_line+max_base), (150,255,255),2)
    ...
    #bottom contour
 #   cv2.line(r_frame,(minX,base_cut+min_line),(minX+minW,base_cut+min_line), (255,30,255),2)
...
#        cv2.line(r_frame,(minX,min_line),(minX+minW,min_line), (255,30,255),2)
    ...
    cv2.line(r_frame,(int(average),0),(int(average),height), (0,255,0),2)
    cv2.line(r_frame,(int(width/2),0),(int(width/2),height), (255,255,0),2)
    cv2.putText(r_frame," Screen centre",(int(width/2),int(height/4)),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255,0), 1) 
    cv2.putText(r_frame," Mean shift",(int(average),int(height/5)),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
