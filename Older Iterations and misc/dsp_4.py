...
def Data(i):
       ...
       #Refresh Plots 
        ...
        graph_data = open('2019_4_2_Full_day_data.txt','r').read()#Minute_history.txt#2019_3_29_Full_day_data.txt
      ...
      #  break_filt=[]
        ...
        lp_drt=lp_drt[(len(lp_drt)-(int(plot_frame_size/2))):len(lp_drt)]
        ...
        Swell_only_peaks, _ = find_peaks(swell_only_plot, height=4, threshold=0,\
        distance=10, prominence=None, width=3, wlen=None, rel_height=0.5)#, plateau_size=None)  
       ...
       # ax4.plot()
        ...
        for peak in Swell_plot_peaks:
            ax2.plot( time_plot[peak],swell_plot[peak], "^",color='y')
            
            
       #period calculations     
        Full_period=int(60/len(Break_plot_peaks))
        Swell_period= 60/len(Swell_only_peaks)  
        ...
        try:
            ...
            Wave heights detected ranges from '+min_h_status+' to '+max_h_status), ha='left',fontsize=15, color='white', rotation=0, wrap=True)    
        except:
           ax4.text(0, 0,'Calibrating...', ha='left', color='white', rotation=0, wrap=True)
            
            
        gold_line = mlines.Line2D([], [], color='y', marker='^',\
       ...
       # ax.set_yticklabels(['T+10','T+20','T+30','T+40','T+50','T+60','T+70','T+80','T+90','T+100'])
        ...
        ax.plot(lp_drt,time_plot,color='y') 
...
ani=animation.FuncAnimation(fig,Data,interval=5000)    
ani=animation.FuncAnimation(fig2,Data,interval=5000)
